...
const useCountryHook = () => {
  ...
  const OffcanvasTitle = "Create Country ";
  ...
  const onSubmit = async (data) => {
    ...
    const response = await CreateDataApi(apiEndpoints?.country, data);
    if (response) {
      fetchUserList();
  ...
  const onUserUpdate = async (data) => {
    try {
      data.updated_by = user.id;
      data.name = `${data?.name}`;
      const response = await UpdateData(apiEndpoints?.countryId(id), data);
      if (response) {
  ...
  const fetchUserList = async (name, status) => {
    ...
    const res = await GetListApi("country", params);
